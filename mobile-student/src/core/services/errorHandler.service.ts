...
export class ErrorHandlerService {
  ...
  static handleError(error: unknown): UserFriendlyError {
    ...
    if (this.isServerError(error)) {
      return {
        ...
        message:
          "Le serveur a rencontré une erreur. Assurez-vous d'être authentifié et réessayez dans quelques instants.",
        code: "SERVER_ERROR",
      };
    }
  ...
  private static extractValidationMessage(error: any): string {
    ...
    return (
      error?.response?.data?.message || "Les données fournies sont invalides."
    );
  }
