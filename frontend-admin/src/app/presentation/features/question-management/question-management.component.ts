...
export class QuestionManagementComponent implements OnInit {
  ...
  getQuestionTypeDescription(type: string | undefined): string {
    ...
    const descriptions: { [key: string]: string } = {
      'CHOIX_MULTIPLE': 'Les étudiants doivent choisir parmi plusieurs options proposées',
      'REPONSE_OUVERTE': 'Les étudiants peuvent répondre librement par du texte'
    };
  ...
  getMultipleChoiceCount(): number {
    return this.questions().filter(q => q.typeQuestion === 'CHOIX_MULTIPLE').length;
  }

  getOpenResponseCount(): number {
    return this.questions().filter(q => q.typeQuestion === 'REPONSE_OUVERTE').length;
  }
